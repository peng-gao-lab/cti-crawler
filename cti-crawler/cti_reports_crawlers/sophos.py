...
from config.cti_reports_crawlers_settings import SOPHOS_BASE_URL, SOPHOS_THREAT_REPORT_BASE_URL, \
    SOPHOS_THREAT_REPORT_DIR, SOPHOS_URL_TO_FILENAME_MAP_PATH


class SophosHTMLCrawler(BaseCrawler):
    def __init__(self):
        super().__init__(SOPHOS_BASE_URL, SOPHOS_THREAT_REPORT_BASE_URL, SOPHOS_THREAT_REPORT_DIR,
                         SOPHOS_URL_TO_FILENAME_MAP_PATH, "sophos_report_crawler")

    def get_report_urls(self):
        page_number = 0  # Maximum 198 confirmed on 07/15/2020
        page_failed_count = 0
        report_urls = []
        ...
        while True:  # For each page
            ...
            try:
                ...
                # title_list = soup.find_all(class_="featured-entry-title dot-ellipsis dot-resize-update "
                #                                   "dot-load-update dot-height-130")
                # if len(title_list) == 0:
